...
//                    options.setAllowBufferHandleOutput(true);
            ...
            d.options.setNumThreads(1);
            d.options.setDeviceType(AiBoostInterpreter.Device.CPU);
            d.options.setQComPowerLevel(AiBoostInterpreter.QCOMPowerLEVEL.QCOM_TURBO);
//            d.options.setNativeLibPath(getApplicationInfo().nativeLibraryDir);
...
//        d.imgData = ByteBuffer.allocateDirect(1 * d.INPUT_SIZE * d.INPUT_SIZE * 3 * numBytesPerChannel);
        d.imgData = d.aiboost.getInputTensor(0);
//        Log.w("imgData", String.valueOf(d.imgData.array().length));
//        Log.w("AiboostimgData", String.valueOf(d.aiboost));
...
//        tfLite.getOutputTensor(0).
        ...
        System.out.println(output.remaining());
        byte[] result = new byte[output.remaining()];
        aiboost.runWithOutInputOutput();
        Log.d("getOutputTensorCount", String.valueOf(aiboost.getOutputTensorCount()));
        output.get(result, 0,  result.length);
        byte[][] labelProbArray = new byte[1][86];;
        //System.arraycopy(result, 0, labelProbArray[0], 0, result.length);
        //Log.d("Aiboost result ", labelProbArray.toString());
